...
import { Wallet, Users, AlertCircle, CheckCircle, Loader2, Search, Clock } from "lucide-react";
...
export function CollectionCenter({
  ...
  const [filters, setFilters] = useState({
    ...
    status: "ALL",
    q: "",
    mode: "DUE_ON" as "DUE_ON" | "DUE_UPTO",
  });
  ...
  const query = useMemo(() => {
    ...
    return sp.toString();
  }, [filters.date, filters.branchId, filters.employeeId, filters.loanType, filters.status, filters.mode, debouncedQ]);
        ...
        <CardContent className="p-4">
          <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-6">
            ...
            <div className="space-y-1">
              ...
              <Select value={filters.mode} onChange={(e) => setFilters((f) => ({ ...f, mode: e.target.value as any }))}>
            ...
            <div className="space-y-1">
              ...
              <Select value={filters.loanType} onChange={(e) => setFilters((f) => ({ ...f, loanType: e.target.value }))}>
            ...
            <TableBody>
              ...
              {rows.map((r) => {
                ...
                return (
                  <TableRow key={r.id}>
                    <TableCell>
                      <div className="font-medium">{r.loanAccount.customer.fullName}</div>
              ...
              {rows.length === 0 && !isPending && (
                <TableRow><TableCell colSpan={11} className="py-12 text-center text-muted-foreground">No dues found for the selected filters.</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </CardContent>
